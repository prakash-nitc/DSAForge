---
title: The DSA | Revision Bible
tagline: Nineteen patterns. Every template in one place. One retention machine.
---

Read [Part 0](Part 0). Run one card. Report the number.

## Where are you today?

| If you are… | What happens | Go to |
| --- | --- | --- |
| About to sit an OA in 24–72 hours | Reactivation only. No new topics. | [24-Hour Protocol](§7.1) [72-Hour Protocol](§7.2) |
| Doing your normal daily prep | Permanent standing routine, non-negotiable. | [The 25-Minute Daily Loop](§7.7) |
| Repairing a pattern that has decayed | One 90-minute session restores 6–20 problems. | [Pick the card](Part II) [The repair session](§5) |
| Building a topic from zero | Graphs, DP, Tries: templates first, then problems, 4–6 days each. | [Topic Builds](Part III) |
| Blanking on Java syntax under pressure | Collections, idioms, traps, complexity. | [Idiom sheet](§19) [The eighteen traps](§20) |
| The night before an interview | Checklist, then the complete template library. | [The night before](Appendix J) [All templates](Appendix A) |

## Pattern board

Solved out of total, from your tracker on 4 September. Open a card to run its repair session. Update the numbers in the table on the Part II overview page and this board follows.

{{pattern-board}}

## Anchors right now

{{anchor-summary}}

## Keep these three close

- [The template library you blank-page from](Appendix A)
- [The anchors you tick](Appendix C)
- [The rules for above your desk](Appendix K)
